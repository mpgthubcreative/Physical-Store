 ...
 * primary key is the cheaper alternative used here instead — a FRESH read,
 * every single call, on every admin-* endpoint.
 *
 * ---- Phase 5D.3 performance investigation (closed) ----
 * A short-lived in-memory cache for this status read was built and measured
 * in production. It was reverted: cache-miss and cache-hit total handler
 * time were essentially identical (~1403ms vs ~1407ms) because the
 * Firestore cost simply moved to whichever call ran first in the
 * invocation, not away — so the cache traded a real security property (an
 * instantly-enforced disable) for no measurable speed gain. Every admin-*
 * endpoint performs a fresh status check again, as below.
 */
